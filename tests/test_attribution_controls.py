"""Tests for Stage 6's controls (scripts/06_attribution_controls.py).
...
controls = _load("stage06", "scripts/06_attribution_controls.py")
...
def test_all_fused_variants_share_one_shape():
    ...
    shapes = {v: controls.build_control_features(d, 19, v, rng).shape
...
def test_answer_variant_is_the_bare_answer_block():
    d = _doc()
    out = controls.build_control_features(d, 19, "answer", np.random.default_rng(0))
...
def test_every_variant_preserves_the_answer_block_untouched():
    ...
    rng = np.random.default_rng(0)
    for v in ("fused_attr", "ctrl_docmean", "ctrl_shuffled", "ctrl_random"):
        out = controls.build_control_features(d, 19, v, rng)
...
def test_docmean_control_is_constant_within_the_document():
    ...
    d = _doc()
    out = controls.build_control_features(d, 19, "ctrl_docmean", np.random.default_rng(0))
    block = out[:, 6:]
...
def test_shuffled_control_is_a_permutation_of_the_same_vectors():
    ...
    out = controls.build_control_features(d, 19, "ctrl_shuffled", np.random.default_rng(1))
    block = out[:, 6:]
...
def test_shuffled_control_actually_moves_rows():
    ...
    moved = False
    for s in range(5):
        block = controls.build_control_features(d, 19, "ctrl_shuffled",
...
def test_single_field_document_survives_every_control():
    ...
    d = _doc(n_fields=1)
    for v in ("fused_attr", "ctrl_docmean", "ctrl_shuffled", "ctrl_random"):
        out = controls.build_control_features(d, 19, v, np.random.default_rng(0))
...
def test_random_control_matches_the_attr_block_scale():
    ...
    out = controls.build_control_features(d, 19, "ctrl_random", np.random.default_rng(0))
    assert np.allclose(out[:, 3:].std(axis=0), 10, rtol=0.25)


def test_unknown_variant_is_rejected():
    with pytest.raises(ValueError):
        controls.build_control_features(_doc(), 19, "nope", np.random.default_rng(0))


# --- statistics ------------------------------------------------------------

def test_holm_is_step_down_and_monotone():
    adj = controls.holm_bonferroni({"a": 0.01, "b": 0.02, "c": 0.04})
...
def test_holm_caps_at_one_and_skips_missing_pvalues():
    adj = controls.holm_bonferroni({"a": 0.9, "b": 0.95, "c": None})
    assert "c" not in adj
    assert all(v <= 1.0 for v in adj.values())
    assert controls.holm_bonferroni({}) == {}
...
def test_bootstrap_ci_brackets_a_real_positive_shift():
    ...
    b = [x + 0.02 for x in a]
    ci = controls.bootstrap_delta_ci(a, b, n_boot=500, seed=0)
...
def test_bootstrap_ci_straddles_zero_when_there_is_no_effect():
    ...
    ci = controls.bootstrap_delta_ci(a, b, n_boot=500, seed=1)
...
def test_bootstrap_ci_ignores_unpaired_documents():
    ...
    ci = controls.bootstrap_delta_ci(a, b, n_boot=200, seed=0)
    assert ci is None or ci["n_pairs"] == 4


def test_bootstrap_declines_on_too_few_pairs():
    assert controls.bootstrap_delta_ci([0.5, 0.6], [0.6, 0.7], n_boot=100) is None
    assert controls.bootstrap_delta_ci([0.5] * 10, [0.6] * 10, n_boot=0) is None


def test_average_per_doc_requires_every_seed_to_be_valid():
    merged = controls.average_per_doc([[0.6, 0.8, None], [0.8, None, 0.5]])
