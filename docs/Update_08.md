...
**The headline comparison is the leave‑one‑document‑out one**, because it is the
only one where probe and baselines are scored on an identical, leakage‑free
field set (Stage 9, pooled out‑of‑fold):
...
So the answer‑only probe beats the best free baseline by **+0.046**, and the full
method by **+0.074**. Both signals are ranked over the same 5,071 fields.

### The per‑layer CV curve is a layer‑selection diagnostic, not a headline

Separately, per‑layer 5‑fold cross‑validation (Figure F2):

| Layer | 1 | 10 | 12 | 16 | 19 | 21 | 28 |
|---|---:|---:|---:|---:|---:|---:|---:|
| CV AUROC | 0.632 | 0.741 | 0.753 | 0.792 | 0.804 | **0.806** | 0.784 |

The shape is the useful part: signal rises with depth and plateaus in the
mid‑to‑late network.

⚠️ **These CV numbers are NOT comparable to the baselines above.** The folds are
stratified over *fields*, not grouped by document, so fields from the same
document appear in both training and validation. That within‑document leakage
inflates the number — which is precisely why LODO exists and why every claim in
this report rests on LODO instead. The CV curve is reported only to show where
in the network the signal lives.
...
Every headline number was traced back through the code before this report was
written. Recording the audit so the answers are on paper, not in someone's head.

**Clean — the LODO path.** `_fit_fold` masks the held‑out document's rows out of
training entirely, and `_standardize` computes mean and standard deviation on
the *training* fold only before applying them to the held‑out fold. No test row
influences the fit, directly or through scaling. Every claim in §5–§7 comes
through this path.

**Clean — the document‑level features.** `ctrl_docmean`, `ctrl_centered` and
`fused_decomposed` average a document's own attribution vectors. Those vectors
are built from the model's own output — its extracted values matched against its
own reasoning trace — and never touch gold labels. At deployment the same
quantity is computable from the document in hand, so this is a legitimate
feature, not label leakage.

**Clean — the baselines.** Token log‑probability scores involve no training, so
there is nothing to leak. In Stage 9 they are ranked over exactly the same
out‑of‑fold field set as the probe.

**Known and disclosed — the per‑layer CV curve.** `StratifiedKFold` splits over
fields rather than documents, so within‑document leakage inflates those numbers
(§4). They are used for layer *selection* and to show where signal lives, never
as a probe‑vs‑baseline comparison.

**Known and disclosed — layer pre‑commitment.** Layer 19 was fixed from the
300‑document pilot, and those 300 documents are a subset of the 994. The layer
choice therefore saw data inside the final evaluation set. Three mitigations: the
effect holds at layer 23 as well as 19; the CV optimum on the full corpus is
actually layer 21, so we are not reporting the best‑case layer; and the paired
tests compare every variant at the *same* layer, so no variant gets a private
choice.

**Known and disclosed — variant selection.** `fused_decomposed` was introduced
after seeing that `fused_attr` matched the document‑mean control. It is a
motivated fix to a specific, identified defect in how the features were
presented to the probe (§6), not a search over variants — but it was not
pre‑registered, and we say so.

**Not applicable — regeneration.** Stage 9 consumes out‑of‑fold probe scores
only. The `oracle` row uses true labels by construction and is explicitly an
